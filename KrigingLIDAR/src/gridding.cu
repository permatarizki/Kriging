...
__global__ void on_core_process(double* ptrIsFinished, double *x_device, double *y_device, double *z_device, double minimumX, double minimumY, int numInputData)
{
...
//		}
	...
	ptrIsFinished[threadID]=idx_onRange;
}
...
int main(void)
{
	...
	char* inputPathLIDARdata = "data/Data3_XYZ_Ground.txt";
	//	char* inputPathLIDARdata = "data/DataSample.txt";

	//Calculate number of line from input file
	//	static const char filename[] = "data/Data3_XYZ_Ground.txt";
	...
	//TODO serialize number of input
	//specify CUDA dimension
	double *x_devices;
	double *y_devices;
	double *z_devices;
	double *isFinished;
	...
	cudaMalloc((void**)&isFinished, sizeof(double)*1000000);
	cudaMemset(isFinished,-1,sizeof(double)*1000000);
	//combination <grid,block> = <1000,100> is well tested
	...
	double isFinished_host[1000000];
	double *ptrFinish;
	ptrFinish = isFinished_host;

	clock_t preprocessing_time = clock();

	on_core_process<<< dimGrid, dimBlock>>>(isFinished, x_devices, y_devices, z_devices,minX,minY, N);
	clock_t finished_time = clock();
	printf("CUDA err: %s \n", cudaGetErrorString(cudaGetLastError()));
	printf("CUDA err: %s \n", cudaGetErrorString(cudaDeviceSynchronize()));

	//	cudaThreadSynchronize();
	cudaMemcpy(ptrFinish,isFinished,sizeof(double)*1000000,cudaMemcpyDeviceToHost);
