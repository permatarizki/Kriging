package wise.algorithm.kriging;
...
public class CalculateDistanceVariogramWeight {	
...
//			}
			...
			//do calculate determinant, save it into matrix det_matrix[]; 
			// Ax = b
			Matrix A = new Matrix(temp_matrix_B);
			Matrix b = new Matrix(temp_matrix_A);
			
			Matrix x = A.solve(b);
			
			System.out.println("row : "+x.getRowDimension());
			System.out.println("column : "+x.getColumnDimension()); //should be 1
			
			int row = x.getRowDimension();
			int column = x.getColumnDimension();
					
			for(i=0;i<row;i++){
				context.write(new Text(key.toString()), new Text(Double.toString(x.get(i, 0))));
			}
			
		}

		//calculate variogram
		public static double semivariogramExponential(double h) {
	...
	public static void main(String[] args) throws Exception {
		...
		int matrix_size = 31;  //this size is K closest Nodes + 1
		conf.set("line.counter", "0");
		conf.set("matrix.size.NbyN", Integer.toString(matrix_size));
		String[] otherArgs = new GenericOptionsParser(conf, args).getRemainingArgs();
		if (otherArgs.length != 2) {
			System.err.println("Usage: wordcount <grid_and_KclosestNodes> <out_distance>");
			System.exit(2);
		}
